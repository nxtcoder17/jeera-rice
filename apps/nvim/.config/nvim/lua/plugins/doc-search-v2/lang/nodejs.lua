...
function M.build_cmd(scope, pattern, opts)
  ...
  if scope == "workspace" then
    ...
    if not awk_path then
      return "echo 'Failed to create awk script'"
    end
  ...
  elseif scope == "dep" then
    ...
    -- Search source files first, fall back to .d.ts
    -- Exclude minified/bundled files
    local src_globs = " --glob '*.ts' --glob '*.js' --glob '*.mjs' --glob '*.tsx' --glob '*.jsx'"
      .. " --glob '!*.d.ts' --glob '!*.min.*' --glob '!dist/**' --glob '!build/**'"
      .. " --glob '!*.bundle.*' --glob '!*.prod.*' --glob '!*.umd.*' --glob '!*.cjs'"
    local dts_globs = " --glob '*.d.ts'"

    local cmds = {}
    for _, dep in ipairs(deps) do
      -- Try source first; if no results, fall back to .d.ts
      local cmd = '(cd "'
        .. dep.dir
        .. '" && '
        .. "{ src=$(rg -n --no-heading -e '"
        .. pattern
        .. "'"
        .. src_globs
        .. " . 2>/dev/null); "
        .. 'if [ -n "$src" ]; then echo "$src"; '
        .. "else rg -n --no-heading -e '"
        .. pattern
        .. "'"
        .. dts_globs
        .. " . 2>/dev/null; fi; }"
