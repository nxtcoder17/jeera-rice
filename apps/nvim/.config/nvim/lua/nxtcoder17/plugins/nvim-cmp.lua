...
cmp.setup({
	...
	mapping = cmp.mapping.preset.insert({
		...
		["<CR>"] = cmp.mapping(function(fallback)
			local line, col = unpack(vim.api.nvim_win_get_cursor(0))
			local c = vim.api.nvim_buf_get_text(0, line - 1, col - 1, line - 1, col, {})
			print("char:", table.concat(c, ""))
			if table.concat(c, "") == "{" then
				fallback()
				return
			end
			if cmp.visible() then
	...
	sources = cmp.config.sources({
		...
		{ name = "cmp_tabnine", priority = 40 },
		{ name = "path", max_item_count = 5, priority = 30 },
	}, {
		-- { name = "cmp_tabnine" },
