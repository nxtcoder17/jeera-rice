...
function M.render()
	...
	if vim.api.nvim_win_get_width(0) < math.floor(vim.o.columns / 3) then
		...
		local special_dirs = {
			JEERA_RICE = vim.fs.joinpath(vim.fn.getenv("HOME"), "me", "jeera-rice"),
...
-- vim.opt.winbar = M.render()

vim.api.nvim_create_autocmd("BufWinEnter", {
	group = vim.api.nvim_create_augroup("ui/winbar", { clear = true }),
	desc = "Attach winbar",
