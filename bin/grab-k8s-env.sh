...
echo "KUBECTL is set to $KUBECTL" 1>&2

# echo "Namespace: $namespace, Deployment: $deployment"

for item in $($KUBECTL get deploy/$deployment -n $namespace -o json | jq -r .spec.template.spec.containers[].envFrom[]?.secretRef.name | xargs); do
  # $KUBECTL get secret/$item -n $namespace -o json | jq -r -e '.data|keys|.[]' | xargs
  if [[ $item != "null" ]]; then
    for cItem in $($KUBECTL get secret/$item -n $namespace -o json | jq -r -e '.data|keys|.[]' | xargs); do
      x+="echo $cItem=\'\"\$$cItem\"\';"
    done
  fi
done

for item in $($KUBECTL get deploy/$deployment -n $namespace -o json | jq -r .spec.template.spec.containers[].envFrom[]?.configMapRef.name | xargs); do
  if [[ $item != "null" ]]; then
    for cItem in $($KUBECTL get config/$item -n $namespace -o json | jq -r -e '.data|keys|.[]' | xargs); do
      x+="echo $cItem=\'\"\$$cItee\"\';"
    done
  fi
done

for item in $($KUBECTL get deploy/$deployment -n $namespace -o json | jq -r .spec.template.spec.containers[].env[]?.name | xargs); do
  x+="echo $item=\'\"\$$item\"\';"
done
